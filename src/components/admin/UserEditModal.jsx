...
const UserEditModal = ({ user: editingUser, onClose, onUpdate }) => {
    const { token } = useAuth();
    const [isLoading, setIsLoading] = useState(false);
    const [actionMessage, setActionMessage] = useState({ type: '', text: '' });
    const [newPlan, setNewPlan] = useState(editingUser.planType);
    ...
    const handleAction = async (endpoint, method = 'POST', body = {}) => {
        ...
        try {
            const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/admin/users/${editingUser.id}/${endpoint}`, {
                method,
    ...
    const handleDeleteUser = async () => {
        ...
        setActionMessage({ type: '', text: '' });
        try {
            const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/admin/users/${editingUser.id}`, {
                method: 'DELETE',
    ...
    const isDeactivated = editingUser.subscriptionStatus === 'deactivated';
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <Card className="max-w-lg w-full relative">
                ...
                <div>
                    <h2 className="text-2xl font-bold">{editingUser.name || 'No Name Provided'}</h2>
                    <p className="text-text-secondary dark:text-dark-text-secondary">{editingUser.email}</p>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    <div className="p-4 bg-slate-100 dark:bg-dark-primary-bg rounded-lg">
                        ...
                        <div className="flex items-center gap-2">
                           ...
                           <button onClick={() => handleAction('plan', 'PUT', { planType: newPlan })} disabled={isLoading || newPlan === editingUser.planType} className="bg-slate-200 dark:bg-slate-700 font-semibold px-4 py-2 rounded-lg text-sm hover:bg-slate-300 dark:hover:bg-slate-600 flex items-center gap-2 disabled:opacity-50">
                                <Save size={16}/> Save
                           </button>
                        </div>
                    </div>
                ...
                <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
                    <Card className="max-w-sm w-full">
                        <div className="flex flex-col items-center text-center">
                            ...
                            <p className="text-text-secondary dark:text-dark-text-secondary">This will permanently delete {editingUser.name} and all their associated data. This action cannot be undone.</p>
