...
const FinanceView = ({ summaryData, filterPeriod, setFilterPeriod, onAnalyze, onExport, onDeleteTransaction }) => {
    return (
                            ...
                            <li key={t.id} className="py-3 sm:py-4 group">
                                <div className="grid grid-cols-4 items-center gap-4">
                                    ...
                                    <div className="inline-flex items-center justify-end text-base font-semibold">
                                        ...
                                        <button onClick={() => onDeleteTransaction(t.id)} className="ml-4 text-text-secondary dark:text-dark-text-secondary hover:text-red-500 opacity-0 group-hover:opacity-100 transition-opacity"><Trash2 size={16} /></button>
                                    </div>
                                </div>
                            </li>
                        ))}
...
        const scope = view === 'invoices' ? 'business' : view;
        const summaryToAnalyze = scope === 'business' ? businessSummary : personalSummary;
        setIsAiLoading(true);
        setAiAnalysis('');
        try {
...
            {view === 'business' && <FinanceView summaryData={businessSummary} filterPeriod={filterPeriod} setFilterPeriod={setFilterPeriod} onAddTransaction={() => setIsTransactionModalVisible(true)} onDeleteTransaction={handleDeleteTransaction} onAnalyze={() => { handleAiAnalysis(); setIsAiModalVisible(true); }} onExport={handleExportCSV} />}
            {view === 'personal' && <FinanceView summaryData={personalSummary} filterPeriod={filterPeriod} setFilterPeriod={setFilterPeriod} onAddTransaction={() => setIsTransactionModalVisible(true)} onDeleteTransaction={handleDeleteTransaction} onAnalyze={() => { handleAiAnalysis(); setIsAiModalVisible(true); }} onExport={handleExportCSV} />}
            
            {view === 'invoices' && (
                <Card>
                    <div className="flex justify-between items-center mb-4">
...
            )}
            
            {isTransferModalVisible && <TransferModal token={token} onClose={() => setIsTransferModalVisible(false)} onTransferSuccess={() => fetchData()} />}
            
            {isInvoiceModalVisible && <CreateInvoiceModal token={token} user={user} clients={clients} onClose={() => setIsInvoiceModalVisible(false)} onInvoiceCreated={fetchData} />}
